...
import {GetPlayerListHomeAction} from '../../../../redux/Actions/GetPlayerListHomeAction';

const widthScreen = Dimensions.get('window').width;
const heightScreen = Dimensions.get('window').height;

const HomeForPlayer = ({navigation}) => {
  const dispatch = useDispatch();
  const GetArticlelistsuccessres = useSelector(
    state => state?.ArticalListReducer?.ARTICALLISTRES,
  );
  const GetPromotionalVideolListsuccessres = useSelector(
    state => state?.GetPromotionalVideolListReducer?.PROMOTIONALVIDEORES,
  );
  const GetPlayerHomeVideolListsuccessres = useSelector(
    state => state?.GetPlayerListHomeReducer?.GETPLAYERHOMERES,
  );
  // console.log('GetPlayerHomelListsuccessres.... ',GetPlayerHomeVideolListsuccessres);
  ...
  useEffect(() => {
    if (token) {
      GetPlayerListVideoListApicall(token);
  ...
  useEffect(() => {
    if (GetPlayerHomeVideolListsuccessres) {
      setIsLoading(false);
    }
  }, [GetPlayerHomeVideolListsuccessres]);
  ...
  const GetPlayerListVideoListApicall = () => {
    if (token) {
      setIsLoading(true);
      const sendData = {
        token: token,
      ...
      dispatch(GetPlayerListHomeAction(sendData));
    }
  };

  const GetPromotionalVideoListApicall = () => {
    if (token) {
      setIsLoading(true);
      const sendData = {
        token: token,
  ...
  const renderFnc1 = item => {
    return (
      <TouchableOpacity>
        <View style={styles.AdminvidView}>
          {/* <Image  style={styles.backgroundVideo} source={{ uri: item.video }}/> */}
          <Video
            paused={true}
    ...
    <View style={styles.mainContainer}>
      ...
      <ScrollView>
        ...
        {/* <View style={styles.carousal}>
          ...
          <FlatList
            data={GetPlayerHomeVideolListsuccessres?.data}
            keyExtractor={item => item?.id}
            renderItem={({item}) => renderFnc1(item)}
            horizontal
          />
          )}
        </View> */}
